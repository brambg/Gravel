...
public class InterpolationCreationHandler implements ShapeCreationMouseHandler {
  ...
  public void setShapeParameters(NURBSCreationMessage nm) {
    if ((nm == null) || (nm.getType() != NURBSCreationMessage.PERIODIC_INTERPOLATION) || (!nm.isValid())) { //Unsuitable, reset to initial Values
      reInit();
      return;
    }
    if (dragged())
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (DragOrigin != null) {
      ...
      if (firstdrag) //Add StartDragpoint
      {
        ...
        {
          if (PointAdditionStatus == NURBSCreationMessage.ADD_BETWEEN) { //In Between
        ...
        if ((DragOriginIndex != -1) && ((MousePosInShape == DragOriginIndex) || (MousePosInShape == -1))) { //Added in Between or Movement - move correct one
          InterpolationPoints.set(DragOriginIndex, exactPointInGraph);
          updateShape();
        } else if (MousePosInShape == -1) { //Added in the end...update last CP
