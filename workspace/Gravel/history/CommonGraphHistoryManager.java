...
public class CommonGraphHistoryManager implements Observer {
  ...
  public void update(Observable o, Object arg) {
    ...
    if ((m.getModifiedElementTypes() == GraphConstraints.SELECTION) && (!trackSelection)) { //Reine Selection Veränderung, aber wir verfolgen sowas nicht -> Update LastGraph
    ...
    } else //Sonst erst auf BlockEnde prüfen
    {
      if ((m.getModification() & GraphConstraints.BLOCK_END) == GraphConstraints.BLOCK_END) //Block endet with this Message
      {
        ...
        if (blockdepth == 0) { //Block_END & not Aborted, so we have a Message to handle
          active = true;
