...
public abstract class CommonGraphAction {
  ...
  protected void exchangeSubgraphMembership(int ItemType, int itemindex, VGraphInterface first, VGraphInterface second) throws GraphActionException {
    ...
    if (first.getType() == VGraphInterface.GRAPH) {
      firstSet = ((VGraph) first).modifySubgraphs;
      firstMSet = ((VGraph) first).getMathGraph().modifySubgraphs;
      secondSet = ((VGraph) second).modifySubgraphs;
      secondMSet = ((VGraph) second).getMathGraph().modifySubgraphs;
      if (ItemType == GraphConstraints.HYPEREDGE)
        return;
    } else if (first.getType() == VGraphInterface.HYPERGRAPH) {
      firstSet = ((VHyperGraph) first).modifySubgraphs;
      firstMSet = ((VHyperGraph) first).getMathGraph().modifySubgraphs;
      secondSet = ((VHyperGraph) second).modifySubgraphs;
      secondMSet = ((VHyperGraph) second).getMathGraph().modifySubgraphs;
