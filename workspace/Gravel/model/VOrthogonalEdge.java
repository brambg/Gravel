...
public class VOrthogonalEdge extends VEdge {
  ...
  public GeneralPath getPath(Point Start, Point End, float zoom) {
    ...
    if (verticalfirst) { //Also zuerst y anpassen
      p.lineTo(Start.x * zoom, End.y * zoom);
      p.lineTo(End.x * zoom, End.y * zoom);
    } else {  //Sonst erst Horizontal also zuerst x
  ...
  public Point getMax() {  //Kein Kontrollpunkt also ein Minimum zur�ckgeben
    return new Point(0, 0);
  }

  public Point getMin() {  //Kein Kontrollpunkt also ein Maximum zur�ckgeben
