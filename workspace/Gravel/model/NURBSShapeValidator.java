...
public class NURBSShapeValidator extends NURBSShape {
  private class PointInfo { //Additional Info for a Point
  ...
  private void prepareResult() {
    ...
    if (ResultValidation) //Nodes are valid due to inside or outside
    {
      ...
      while (nodeiter.hasNext()) //Iterator for all node-positions
      {
        ...
        } else if (!Outsets.contains(pointInformation.get(pos).set))
  ...
  private void checkDistances() {
    ...
    while (nodeiter.hasNext()) //Iterator for all node-positions
    {
      int id = nodeiter.next().index;
      Point2D pos = getPointOfNode(id);
      if (vG.getMathGraph().modifyHyperEdges.get(HEIndex).containsNode(id)) { //Inside so its radius (distance to projecion must be at most minDist
