  ...
  protected int prepareFragment(double t1, double t2) {
    ...
    if (t1 < t2) {
      ...
      if (minNum <= 0) {
        int count = -minNum + 2;
        Refinement.clear();
        for (int i = 1; i < count; i++) { //TODO: Find nonlinear refinement that produces more points at start/end then in the middle
    ...
    } else //if we are in the area of circular movement and we have to think of moving over start/end the formular is a little bit different and mor difficult
    {
      ...
      if (minNum <= 0) {
        ...
        int i = 1;
        while ((i < count) && ((t1 + (newt2 - t1) * (double) i / (double) count) <= Knots.get(maxKnotIndex - degree))) { //Refine at the end
