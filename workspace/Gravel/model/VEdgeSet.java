...
public class VEdgeSet extends Observable implements Observer {
   ...
   *
   * @param edge       the new VEdge
   * @param medge      mathematical elements of the new edge, if its index differs, this index is ignored
   * @param startPoint
   * @param endPoint
   */
  public void add(VEdge edge, MEdge medge, Point start, Point end) {
    ...
    if (mG.modifyEdges.add(medge)) //succesfull added in MathGraph
    {
      ...
      try {
        ...
        if ((medge.StartIndex != medge.EndIndex) && (mG.isDirected()) && (mG.modifyEdges.cardinalityBetween(medge.EndIndex, medge.StartIndex) == 1) && (get(mG.modifyEdges.indicesBetween(medge.EndIndex, medge.StartIndex).firstElement()).getEdgeType() == VEdge.STRAIGHTLINE) && (edge.getEdgeType() == VEdge.STRAIGHTLINE)) { //Dann würde diese Kante direkt auf der anderen liegen
   ...
   *
   * @param i index of the searched edge
   ...
   *
   * @return a Iterator of type VEdge
   */
   ...
   *
   * @param m         a Point
   * @param variation the distance from the point m
