...
public class GravelMLContentHandler implements ContentHandler {
    ...
    if (Status == PARSE_EDGES) {
      if (isVisual) {
        if ((vG == null) || (vG.getMathGraph().modifyNodes.cardinality() == 0)) {
          isValid = false;
          main.DEBUG.println(main.DEBUG.MIDDLE, "No VGraph or no Nodes existent. Can't Parse Edges");
        }
      } else if (mG == null) {
        isValid = false;
        main.DEBUG.println(main.DEBUG.MIDDLE, "No MGraph existent. Can't Parse Edges");
      }
    } else if (Status == PARSE_SUBGRAPHS) {
      if (isVisual) {
        if ((vG == null)) {
          isValid = false;
          main.DEBUG.println(main.DEBUG.MIDDLE, "No VGraph existent. Can't Parse Subgraphs/SubSets");
        }
      } else if (mG == null) {
        isValid = false;
        main.DEBUG.println(main.DEBUG.MIDDLE, "No MGraph existent. Can't Parse Subgraphs/SubSets");
      }
    }
  ...
  private void EndElementData() {
    if (Status == PARSE_NODES) {
      ...
      if (data_key.equals("ev")) //Edge Value
        try {
          ev = Integer.parseInt(CDATA);
        } catch (Exception e) {
          ev = 0;
        }
      if (data_key.equals("ew")) //Edge Width
        try {
          ew = Integer.parseInt(CDATA);
        } catch (Exception e) {
          ew = 0;
        }
      ...
      if (data_key.equals("etd")) //Edge Text Distance
        try {
          etxt.setDistance(Integer.parseInt(CDATA));
        } catch (Exception e) {
          etxt.setDistance(gp.getIntValue("edge.text_distance"));
        }
      if (data_key.equals("etp")) //Edge Text Position
        try {
          etxt.setPosition((float) Integer.parseInt(CDATA) / 100f);
        } catch (Exception e) {
          etxt.setPosition(gp.getFloatValue("edge.text_position"));
        }
      if (data_key.equals("ets")) //Edge Text Size
        try {
          etxt.setSize(Integer.parseInt(CDATA));
        } catch (Exception e) {
          etxt.setSize(gp.getIntValue("edge.text_size"));
        }
      if (data_key.equals("etv")) //Edge Text visible
        try {
          etxt.setVisible(Boolean.parseBoolean(CDATA));
        } catch (Exception e) {
          etxt.setVisible(gp.getBoolValue("edge.text_visible"));
        }
      if (data_key.equals("etw")) //Edge Text Show Value or text
        try {
          etxt.setshowvalue(Boolean.parseBoolean(CDATA));
        } catch (Exception e) {
          etxt.setshowvalue(gp.getBoolValue("edge.text_showvalue"));
        }
      if (data_key.equals("eld")) //Edge Line Distance
        try {
          eline.setDistance(Integer.parseInt(CDATA));
        } catch (Exception e) {
          eline.setDistance(gp.getIntValue("edge.line_distance"));
        }
      if (data_key.equals("ell")) //Edge Line Length
        try {
          eline.setLength(Integer.parseInt(CDATA));
        } catch (Exception e) {
          eline.setLength(gp.getIntValue("edge.line_distance"));
        }
      if (data_key.equals("elt")) //Edge Line Type
        try {
          eline.setType(Integer.parseInt(CDATA));
        } catch (Exception e) {
          eline.setType(gp.getIntValue("edge.line_type"));
        }

      if (data_key.equals("elol")) //Edge Loop Length
        try {
          elol = Integer.parseInt(CDATA);
        } catch (Exception e) {
          elol = gp.getIntValue("edge.loop_length");
        }
      if (data_key.equals("elod")) //Edge Loop Direction
        try {
          elod = Integer.parseInt(CDATA);
        } catch (Exception e) {
          elod = gp.getIntValue("edge.loop_direction");
        }
      if (data_key.equals("elop")) //Edge Loop Proportion
        try {
          elop = Double.parseDouble(CDATA);
        } catch (Exception e) {
          elop = ((double) gp.getIntValue("edge.loop_proportion")) / 100.0d;
        }
      if (data_key.equals("eloc")) //Edge Loop Clockwise
        try {
          eloc = Boolean.parseBoolean(CDATA);
        } catch (Exception e) {
          eloc = gp.getBoolValue("edge.loop_clockwise");
        }

      if (data_key.equals("es")) //Edge Arrow Size
        try {
          es = Float.parseFloat(CDATA);
        } catch (Exception e) {
          es = 0.0f;
        }
      if (data_key.equals("ep")) //Edge Arrow Part
        try {
          ep = Float.parseFloat(CDATA);
        } catch (Exception e) {
          ep = 0.0f;
        }
      if (data_key.equals("eapos")) //Edge Arrow Part
        try {
          eapos = Float.parseFloat(CDATA);
        } catch (Exception e) {
          eapos = -1.0f;
        }
      if (data_key.equals("ea")) //Edge Arrow Alpha
        try {
          ea = Float.parseFloat(CDATA);
        } catch (Exception e) {
          ea = 0.0f;
        }
      ...
      if (data_key.equals("ex")) //Edge ControlPoint X
        try {
          ex = Integer.parseInt(CDATA);
        } catch (Exception e) {
          ex = -1;
        }
      if (data_key.equals("ey")) //Edge ControlPoint X
        try {
          ey = Integer.parseInt(CDATA);
        } catch (Exception e) {
          ey = -1;
        }
      else if (data_key.equals("eo")) //Node Name Visible
        try {
          eo = Boolean.parseBoolean(CDATA);
        } catch (Exception e) {
          eo = gp.getBoolValue("edge.oorth_verticalfirst");
        }
      if ((ex != -1) && (ey != -1)) {  //On ex and ey the order is important !!
        edgepath.add(new Point(ex, ey));
        ex = -1;
        ey = -1;
      }
    } else if (Status == PARSE_SUBGRAPHS) {
  ...
  private void startElementNode(Attributes atts) {
    ...
    } else if (position.equals("graphml.graph")) //Beginning Graph
    {
      ...
      nr = -1;
      nns = -1;
      nnv = gp.getBoolValue("node.name_visible");
